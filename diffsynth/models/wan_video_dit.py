...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs):
        ...
        x = x + gate_msa * self.self_attn(input_x, freqs[:input_x.shape[1]])
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        # Control Input
        x_c, (f_c, h_c, w_c) = self.patchify(x_c)
        ...
        freqs_c = torch.cat([
            self.freqs[0][:f_c].view(f_c, 1, 1, -1).expand(f_c, h_c, w_c, -1),
            self.freqs[1][:h_c].view(1, h_c, 1, -1).expand(f_c, h_c, w_c, -1),
            self.freqs[2][:w_c].view(1, 1, w_c, -1).expand(f_c, h_c, w_c, -1)
        ...
        for idx, block in enumerate(self.blocks):
            if self.training and use_gradient_checkpointing:
                if use_gradient_checkpointing_offload:
                    ...
                    x = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(block),
                        x, context, t_mod, freqs, # [1, 1560, 1536], [1, 512, 1536], [1, 6, 1536], [1560, 1, 64]
                        use_reentrant=False,
                    ) # x: [1, 1560, 1536]
                    if idx < self.num_control_layers:
                        x_c = torch.utils.checkpoint.checkpoint(
